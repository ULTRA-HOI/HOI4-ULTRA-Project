...
NGame = {
	...
	SAVE_VERSION = 31,								-- 1.18.0 (Case Green)
	...
	GAME_SPEED_SECONDS = { 1.4, 1.0, 0.5, 0.25, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
...
NDiplomacy = {
	...
	TENSION_PEACE_FACTOR = 0.4,					-- scale of the amount of tension (from war declaration) reduced when peace is completed. Scaled by current world threat * TENSION_PEACE_FACTOR_THREAT_FACTOR
	...
	VICTORY_POINT_WORTH_FACTOR = 25,--was 10				-- multiplier when calcualting province worth (surrender)
...
NCountry = {
	...
	BASE_COMMAND_POWER_GAIN = 0.6,					-- base value for daily command power gain
...
NProduction = {
	...
	POWERED_FACTORY_SPEED = 0.65, --was 5 --Powered factory speed multiplier.
	POWERED_FACTORY_SPEED_MIL = 1, --was 4.5 --Powered factory speed multiplier.
	POWERED_FACTORY_SPEED_NAV = 1.2, --was 2.5 --Powered factory speed multiplier.
...
NMilitary = {
	...
	MAX_HQ_BATTALION_WIDTH = 2,			-- Max width of regiments in division designer (Army HQ templates).
	MAX_HQ_BATTALION_HEIGHT = 2,			-- Max height of regiments in division designer (Army HQ templates).
	...
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.07, --was 0.060 --global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
	...
	RELIABILTY_RECOVERY = 0.6,                     -- factor affecting how much equipment is returned "from the dead"
	...
	BASE_FORT_PENALTY = -0.15, --was -0.15 --fort penalty
	...
	ANTI_AIR_TARGETTING_TO_CHANCE = 0.04, --was 0.07 --Balancing value to determine the chance of ground AA hitting an attacking airplane, affecting both the effective average damage done by AA to airplanes, and the reduction of damage done by airplanes due to AA support
	...
	EQUIPMENT_COMBAT_LOSS_FACTOR = 0.7, --was 0.70 --% of equipment lost to strength ratio in combat, so some % is returned if below 1
	...
	TRAINING_ATTRITION = 0.02, --was 0.05 --amount of extra attrition from being in training
	...
	PLAN_EXECUTE_RUSH = -10, --was -200 			--When looking for an attack target, this score limit is required in the battle plan to consider province for attack
	...
	COMBAT_OVER_WIDTH_PENALTY = -1, --was -1.5 --over combat width penalty per %.
	...
	STRATEGIC_SPEED_INFRA_BASE = 2.0, --was 5.0 --Base speed of strategic redeployment when not on railways
	STRATEGIC_SPEED_INFRA_MAX = 1.5, --was 10.0 --Additional speed of strategic redeployment on max-level infrastructure
...
NAir = {
	...
	AIR_WING_BOMB_DAMAGE_FACTOR = 1.2, --was 2 --Used to balance the damage done while bombing.
...
NNavy = {
	...
	CAPITAL_SHIP_COMBAT_RETREAT_MULT = 1.0,	 --was 0.5						-- Multiplier on combat retreat threshold for capital ships and carriers (they can take more punishment)
	...
	NAVAL_INVASION_PREPARE_DAYS = 14,								-- base days needed to prepare an invasion
	...
	SUPPLY_NEED_FACTOR = 315, --multiplies supply usage
	...
	MIN_SHIPS_FOR_HIGHER_SHIP_RATIO_PENALTY                         = 8,    -- the minimum fleet size in ships that a fleet must be before having the large fleet penalty applied to them
	...
	SUBMARINE_BASE_TORPEDO_REVEAL_CHANCE 							= 0.18, -- /deprecated, will be removed in the future update/ was 0.035 --Chance of a submarine being revealed when it fires. 1.0 is 100%. this chance is then multiplied with modifier created by comparing firer's visibiility and target's detection
	...
	NAVY_PIERCING_THRESHOLD_CRITICAL_VALUES = {	-- 0 armor will always receive maximum damage (so add overmatching at your own peril). the system expects at least 2 values, with no upper limit.
		...
		-0.95,
		-0.98,
		-1.00 -- For criticals, you could reduce crit chance unlike damage in army combat, but we do not for now.
	},
...
NAI = {
	...
	PRODUCTION_EQUIPMENT_SURPLUS_FACTOR = 0.25, --was 0.8 --Base value for how much of currently used equipment the AI will at least strive to have in stock
	...
	EQUIPMENT_MARKET_BASE_MARKET_RATIO = 0.05,                       -- was 0.2 -- The AI tries to keep ca this ratio of equipment surplus for sale on the market. Gets modified by equipment_market_for_sale_factor AI strategy.
	...
	DEFAULT_MODULE_VARIANT_CREATION_XP_CUTOFF_NAVY = 10,	-- Same as above but for the ship designer.
	...
	STOP_TRAINING_ACTIVE_COMBAT_RATIO = 0.30,            -- ai halts all training when more than this share of its divisions are in active combat (reinforce instead)
	...
	MAX_AIR_REGIONS_TO_CARE_ABOUT = 32,							-- Number of regions we'll consider when trying to split planes a bit. Split is NOT equal, just a guide, leftovers still applied elsewhere if needed
	...
	ASSIGN_INVASION_AMPHIBIOUS_ATTACK_FACTOR = 500.0,            -- Importance of unit's amphibious attack adjuster when assigning to an invasion order
	...
	ASSIGN_TANKS_TO_WAR_FRONT = 30.0,           -- was 6.0      -- Scoring factor for assigning divisions with 'role = armor' or 'front_role_override = offence' to active war fronts
	ASSIGN_TANKS_TO_NON_WAR_FRONT = 0.01,      -- was 0.4       -- Scoring factor for assigning divisions with 'role = armor' or 'front_role_override = offence' to non-war fronts
	...
	MIN_INVASION_UNITS_READY_TO_EXECUTE = 0.75,              -- ai will only activate invasions if this ratio of assigned units are ready
...
NSupply = {
	...
	SUPPLY_POINTS_PER_TRAIN = 18.0, --was 1.0 --old default 1.25 -- Amount of supply that can fit in a train. (Trains distribute supply from capital to a supply node.)
	...
	NON_CORE_MANPOWER_STATE_SUPPLY = 1.2,						--Factor for population sttate supply when controlled by an occupier (NO TAKE FOOD)
...
NIndustrialOrganisation = {
	ASSIGN_DESIGN_TEAM_PP_COST_PER_DAY = 0.01,					-- Cost in Political Power daily generation when one MIO is assigned to a research slot. If 0, cost is entirely disabled.
	...
	DESIGN_TEAM_CHANGE_XP_COST = 0,								-- Flat cost added to the XP cost of a new equipment design. If 0, cost is entirely disabled. If not 0, the checkbox for automatically apply MIO upgrade does not appear, and will be automatically on with no way to turn off
...
NDoctrines = {
        ...
        MASTERY_BANK_CONVERSION_RATE = 0.5,                    -- The rate at which mastery gained when a track is finished or empty is "banked"
